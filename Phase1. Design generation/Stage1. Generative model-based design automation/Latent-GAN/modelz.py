...
class ZGAN(object):
	def __init__(self, sess, is_training = False, z_vector_dim=256, z_dim=256, df_dim=2048, gf_dim=2048, dataset_name='default', checkpoint_dir=None, sample_dir=None, data_dir='./data'):
		"""
		Args:
			too lazy to explain
		"""
	...
	def train(self, config):
		...
		for epoch in range(counter, config.epoch+1):
			...
			errD_temp = errD_total / batch_num
			errG_temp = errG_total / batch_num

			index_list.append(epoch)
			errD_list.append(errD_temp)
			errG_list.append(errG_temp)
			...
			if epoch%1000 == 0:
				...
				print("[Visualized Z]")
		print(errG_list)
		result = dict(epochs = index_list, err_D = errD_list, err_G = errG_list)
		df = pd.DataFrame(result)
		df.to_excel(self.dataset_namez +  "_err" + ".xlsx")
