...
def get_points_from_vox(q, name_list):
	...
	for idx in range(name_num):
		...
		try:
			...
			voxel_model_128 = np.load(name_list[idx][1])
		except:
		...
		# 	for j in range(16):
		# 		for k in range(16):
		# 			voxel_model_128[i*16:i*16+16,j*16:j*16+16,k*16:k*16+16] = voxel_model_b[voxel_model_bi[i,j,k]]
		...
		#vertices, triangles = mcubes.marching_cubes(voxel_model_128, 0.5)
		...
		dim_voxel = 128
		top_view = np.max(voxel_model_128, axis=1)
		...
		#compress model 128 -> 64
		dim_voxel = 64
		voxel_model_temp = np.zeros([dim_voxel,dim_voxel,dim_voxel],np.uint8)
		multiplier = int(128/dim_voxel)
		halfie = int(multiplier/2)
		for i in range(dim_voxel):
			for j in range(dim_voxel):
				for k in range(dim_voxel):
					voxel_model_temp[i,j,k] = np.max(voxel_model_128[i*multiplier:(i+1)*multiplier,j*multiplier:(j+1)*multiplier,k*multiplier:(k+1)*multiplier])
			...
			for i in temp_range:
				...
				for k in temp_range:
					...
					if (np.max(voxel_model_temp[i-1:i+2,j-1:j+2,k-1:k+2])!=np.min(voxel_model_temp[i-1:i+2,j-1:j+2,k-1:k+2])):
						si,sj,sk = sample_point_in_cube(voxel_model_128[i*multiplier:(i+1)*multiplier,j*multiplier:(j+1)*multiplier,k*multiplier:(k+1)*multiplier],voxel_model_temp[i,j,k],halfie)
...
if __name__ == '__main__':
	...
	#dir of voxel models
	voxel_input = r"./ex_data/npy/"  # Put your custom data address

	#name of output file
	hdf5_path = class_name+'/'+class_name+'_vox128.hdf5'

	#obj_list
	fout = open(class_name+'/'+class_name+'_vox128.txt','w',newline='')
	...
	image_list = [file for file in os.listdir(voxel_input) if file.endswith('.npy')]
	print(image_list)
	name_list = []
	...
	for i in range(num_of_process):
		...
		for j in range(i,name_num,num_of_process):
			list_of_names.append([j, voxel_input+str(name_list[j])+".npy"])
