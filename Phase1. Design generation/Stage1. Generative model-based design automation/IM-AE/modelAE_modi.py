...
class IM_AE(object):
	def __init__(self, config):
		...
		self.dataset_load = self.dataset_name
		self.dataset_load_train = self.dataset_name + '_train'
		self.dataset_load_test = self.dataset_name + '_test'

		##
		self.checkpoint_dir = config.checkpoint_dir
		self.data_dir = config.data_dir

		data_hdf5_name_train = self.data_dir+'/'+self.dataset_load_train+'.hdf5'
		data_hdf5_name_test = self.data_dir+'/'+self.dataset_load_test+'.hdf5'
		data_hdf5_name = self.data_dir+'/'+self.dataset_load+'.hdf5'


		if os.path.exists(data_hdf5_name):
			data_dict = h5py.File(data_hdf5_name, 'r')
			self.data_points = (data_dict['points_'+str(self.sample_vox_size)][:].astype(np.float32)+0.5)/128-0.5
		...
		if os.path.exists(data_hdf5_name_train):
			data_dict_train = h5py.File(data_hdf5_name_train, 'r')
			self.data_points_train = (data_dict_train['points_' + str(self.sample_vox_size)][:].astype(
				np.float32) + 0.5) / 128 - 0.5  # 256 ->128 (2023.03.29)
			self.data_values_train = data_dict_train['values_' + str(self.sample_vox_size)][:].astype(np.float32)
			self.data_voxels_train = data_dict_train['voxels'][:]
			self.load_point_batch_size_train = self.data_points_train.shape[1]
			# reshape to NCHW
			self.data_voxels_train = np.reshape(self.data_voxels_train,[-1, 1, self.input_size, self.input_size, self.input_size])
		else:
			print("error: cannot load " + data_hdf5_name_train)
			exit(0)

		if os.path.exists(data_hdf5_name_test):
			data_dict_test = h5py.File(data_hdf5_name_test, 'r')
			self.data_points_test = (data_dict_test['points_'+str(self.sample_vox_size)][:].astype(np.float32)+0.5)/128-0.5 #256 ->128 (2023.03.29)
			self.data_values_test = data_dict_test['values_'+str(self.sample_vox_size)][:].astype(np.float32)
			self.data_voxels_test = data_dict_test['voxels'][:]
			self.load_point_batch_size_test = self.data_points_test.shape[1]
			#reshape to NCHW
			self.data_voxels_test = np.reshape(self.data_voxels_test, [-1,1,self.input_size,self.input_size,self.input_size])
		else:
			print("error: cannot load "+data_hdf5_name_test)
			exit(0)
	...
	def train(self, config):
		...
		shape_num_train = len(self.data_voxels_train)
		shape_num_test = len(self.data_voxels_test)
		batch_index_list = np.arange(shape_num_train)

		print("\n\n----------net summary----------")
		print("training samples   ", shape_num_train)
		print("test samples   ", shape_num_test)
		print("-------------------------------\n\n")

		start_time = time.time()
		assert config.epoch == 0 or config.iteration == 0
		training_epoch = config.epoch + int(config.iteration / shape_num_train)
		batch_num = int(shape_num_train / self.shape_batch_size)
		point_batch_num = int(self.load_point_batch_size / self.point_batch_size)
		data_len=int(shape_num_test)
		index_list = list()
		loss_list = list()
		loss_test_list = list()
		...
		for epoch in range(0, training_epoch):
			...
			for idx in range(batch_num):
				...
				batch_voxels = self.data_voxels_train[dxb].astype(np.float32)
				if point_batch_num == 1:
					point_coord = self.data_points_train[dxb]
					point_value = self.data_values_train[dxb]
				else:
			...
			for idx1 in range(data_len):
				...
				# dxb = batch_index_list[idx*self.shape_batch_size:(idx+1)*self.shape_batch_size]
				batch_voxels_test = self.data_voxels_test[idx1].astype(np.float32)
				point_coord_test = self.data_points_test[idx1]
				point_value_test = self.data_values_test[idx1]

				batch_voxels_test = torch.from_numpy(batch_voxels_test)
				point_coord_test = torch.from_numpy(point_coord_test)
				point_value_test = torch.from_numpy(point_value_test)

				batch_voxels_test = batch_voxels_test.to(self.device)
				point_coord_test = point_coord_test.to(self.device)
				point_value_test = point_value_test.to(self.device)

				self.im_network.zero_grad()

				batch_voxels_test=batch_voxels_test.unsqueeze(dim=0)
				point_coord_test = point_coord_test.unsqueeze(dim=0)
				_, net_out = self.im_network(batch_voxels_test, None, point_coord_test, is_training=True)
				loss_test = self.loss(net_out, point_value_test)
				sum_loss_test += loss_test.item()

			loss_list.append(avg_loss_sp / avg_num)
			loss_test_list.append(sum_loss_test/data_len)
			index_list.append(epoch)

			print(str(self.sample_vox_size) + " Epoch: [%2d/%2d] time: %4.4f, training_loss_sp: %.6f, test_loss_sp: %.6f " % (
			epoch, training_epoch, time.time() - start_time, avg_loss_sp / avg_num, sum_loss_test/data_len))
			if epoch % 10 == 9:
			...
			if epoch % 20 == 19:
				...
				fout.close()
				if epoch % 100 == 99:
					save_ckp_dir = os.path.join(self.checkpoint_path, 'ckps',
												self.checkpoint_name + str(self.sample_vox_size) + "-" + str(
													epoch) + ".pth")
					torch.save(self.im_network.state_dict(), save_ckp_dir)
		...
		for i in range(self.max_to_keep):
			...
			if self.checkpoint_manager_list[pointer] is not None:
				fout.write(self.checkpoint_manager_list[pointer] + "\n")
		fout.close()
		result = dict(epochs=index_list, loss=loss_list, test_loss = loss_test_list)
		df = pd.DataFrame(result)
		df.to_excel(config.sample_dir + "/" + config.dataset + "_loss" + ".xlsx")
	...
	def get_z(self, config):
		...
		hdf5_path = self.checkpoint_dir+'/'+self.model_dir+'/'+self.dataset_load_train+'_z.hdf5'
	...
	def test_z(self, config):
		...
		dataset_namez = 'data_z'

		if os.path.exists(os.path.join(DATA_PATH, dataset_namez + '.hdf5')):
			print(os.path.join(DATA_PATH, dataset_namez + '.hdf5'))
			data_dict = h5py.File(os.path.join(DATA_PATH, dataset_namez + '.hdf5'), 'r')
			data_z = data_dict['zs'][:]

		if os.path.exists(os.path.join(config.sample_dir)):
			os.makedirs(os.path.join(config.sample_dir, dataset_namez + '_stl'), exist_ok=True)

		order_list = list()
		novelty_list = list()
		for t in range(len(data_z)):
			model_z = data_z[t:t + 1]
			...
			vertices = (vertices.astype(np.float32) - 0.5) / self.real_size - 0.5
			# vertices = self.optimize_mesh(vertices,model_z)
			write_ply_triangle(config.sample_dir + "/" + "z_out" + str(t) + ".ply", vertices, triangles)
			# time.sleep(1)
			mesh = o3d.io.read_triangle_mesh(config.sample_dir + "/" + "z_out" + str(t) + ".ply")
			poisson_mesh = o3d.geometry.TriangleMesh.compute_triangle_normals(mesh)
			o3d.io.write_triangle_mesh(
				config.sample_dir + "/" + f"{dataset_namez}_stl" + "/" + "z_out" + str(t) + ".stl", poisson_mesh)
			print("[sample Z]")
